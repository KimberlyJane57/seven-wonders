...
fetch(
  "http://localhost:8080/https://api.content.tripadvisor.com/api/v1/location/search?key=29CB0A3EE8A74953A71A7964758809DA&searchQuery=Perito%20Moreno%20Glacier%2C%20Argentina&category=Hotels&language=en",
  options
)
...
function tripAdvisorDetails(data) {
  for (var i = 0; i < 5; i++) {
    if (
      data[i].location_id !== "15005432" &&
      data[i].location_id !== "11987921" &&
      data[i].location_id !== "11466641" &&
      data[i].location_id !== "11452389"
    ) {
      fetch(
